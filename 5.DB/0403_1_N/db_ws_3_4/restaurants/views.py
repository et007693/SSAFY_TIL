...
from .forms import RestaurantForm, MenuForm, UpdateForm
...
def update(request, restaurant_pk, menu_pk):
    restaurant = Restaurant.objects.get(pk=restaurant_pk)
    menu = Menu.objects.get(pk=menu_pk)
    if request.method == "POST":
        form = UpdateForm(request.POST, instance=menu)
        if form.is_valid():
            form.save()
            return redirect('restaurants:detail', restaurant_pk)
    else:
        form = UpdateForm(instance=menu)
    context = {
        'restaurant':restaurant,
        'form':form
    }
